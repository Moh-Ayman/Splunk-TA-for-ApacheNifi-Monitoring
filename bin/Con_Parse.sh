...
grep -v "^-|Connection ID|^20" /tmp/Connection.stat |sed -e 's/,/;/g'|awk -F"|" '{print $2","$3","$4","$5","$6","$7","$8}' | sed -e's/  */ /g' | grep -v "^,"|grep -v "Connection ID , Source , Connection Name" |awk -v var="${CONNECTION_REPORTING_DATE_MOD}" '{print var","$0}'
